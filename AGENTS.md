...
The repository is a small Perl 5 engine built around modules in `Chess/`. `Chess::State` owns board representation and FEN parsing, `Chess::Engine` implements search, `Chess::Book` supplies opening moves, and `Chess::Constant` plus `Chess::LocationModifer` define enumerations and square bonuses. CLI entry points live at the root: `play.pl` drives an interactive game and `uci.pl` exposes a UCI interface. Perft regression lives at `tests/perft.pl` (`perft.pl` at repo root is a compatibility shim). Keep new scripts alongside these so `FindBin` can locate local modules.
...
- `perl tests/perft.pl 4` — runs depth-limited node counts from the default FEN; pass a second argument (0–4) to load canned stress positions.
